  ...
  class CSF_Shortcoder extends CSF_Abstract{
    ...
    public function __construct( $key, $params = array() ) {
      ...
      if ( ! empty( $this->args['show_in_editor'] ) ) {
        ...
        SPLW::$shortcode_instances[$this->unique] = wp_parse_args( array( 'hash' => md5( $key ), 'modal_id' => $this->unique ), $this->args );
    ...
    ?>
      <div id="splwt-lite-modal-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui splwt-lite-modal splwt-lite-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-modal-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'csf_shortcode_nonce' ) ); ?>">
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'csf_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            $button_title    = ( ! empty( $section['button_title'] ) ) ? ' '. $section['button_title'] : esc_html__( 'Add one more', 'splwt-lite' );
        ...
        echo '<div class="splwt-lite-field splwt-lite-error-text">'. esc_html__( 'Error: Nonce verification has failed. Please try again.', 'splwt-lite' ) .'</div>';
      }
