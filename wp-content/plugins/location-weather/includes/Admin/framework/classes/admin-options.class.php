...
if ( ! class_exists( 'CSF_Options' ) ) {
  class CSF_Options extends CSF_Abstract {
    ...
    public function ajax_save() {
      ...
      if ( ! $result ) {
        wp_send_json_error( array( 'error' => esc_html__( 'Error while saving the changes.', 'splwt-lite' ) ) );
      } else {
    ...
    public function set_options( $ajax = false ) {
      ...
      if ( wp_verify_nonce( $nonce, 'csf_options_nonce' ) ) {
        ...
        if ( ! $ajax && ! empty( $response[ 'csf_import_data' ] ) ) {
          ...
          $importing    = true;
          $this->notice = esc_html__( 'Settings successfully imported.', 'splwt-lite' );
        ...
        if ( ! empty( $transient['reset'] ) ) {
          ...
          $this->notice = esc_html__( 'Default settings restored.', 'splwt-lite' );
        ...
        if ( empty( $this->notice ) ) {
          $this->notice = esc_html__( 'Settings saved.', 'splwt-lite' );
        }
    ...
    public function add_options_html() {
      ...
      echo '<div class="splwt-lite splwt-lite-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<input type="hidden" class="splwt-lite-section-id" name="csf_transient[section]" value="1">';
        ...
        echo '<div class="splwt-lite-header-inner">';
          $_menu_icon = file_get_contents( LOCATION_WEATHER_ASSETS . '/font/lw-icon.svg' );
          echo '<div class="splwt-lite-header-left">';
          echo '<img class="splwt-setting-logo" src="'. 'data:image/svg+xml;base64,' . base64_encode( $_menu_icon).'"/><h1>' . $this->args['framework_title'] . '</h1>';
          echo '</div>';
          ...
          echo '<div class="splwt-lite-header-right">';
            ...
            echo ( $this->args['show_form_warning'] ) ? '<div class="splwt-lite-form-result splwt-lite-form-warning">'. esc_html__( 'You have unsaved changes, save your changes!', 'splwt-lite' ) .'</div>' : '';

            echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="splwt-lite-expand-all" title="'. esc_html__( 'show all settings', 'splwt-lite' ) .'"><i class="fas fa-outdent"></i></div>' : '';

            echo ( $this->args['show_search'] ) ? '<div class="splwt-lite-search"><input type="text" name="splwt-lite-search" placeholder="'. esc_html__( 'Search...', 'splwt-lite' ) .'" autocomplete="off" /></div>' : '';

            echo '<div class="splwt-lite-buttons">';
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary splwt-lite-top-save splwt-lite-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'splwt-lite' ) .'" data-save="'. esc_html__( 'Saving...', 'splwt-lite' ) .'">';
            echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary splwt-lite-reset-section splwt-lite-confirm" value="'. esc_html__( 'Reset Section', 'splwt-lite' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'splwt-lite' ) .'">' : '';
            echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button splwt-lite-warning-primary splwt-lite-reset-all splwt-lite-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'splwt-lite' ) : esc_html__( 'Reset', 'splwt-lite' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'splwt-lite' ) .'">' : '';
            echo '</div>';
