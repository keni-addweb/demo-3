...
/*     public static function createCustomizeOptions( $id, $args = array() ) {
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      $path     = '';
      $file     = ltrim( $file, '/' );
      $override = apply_filters( 'csf_override', 'splwt-lite-override' );
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      // Main style
      wp_enqueue_style( 'splwt-lite', self::include_plugin_url( 'assets/css/style'. $min .'.css' ), array(), self::$version, 'all' );

      // Main RTL styles
      if ( is_rtl() ) {
        wp_enqueue_style( 'splwt-lite-rtl', self::include_plugin_url( 'assets/css/style-rtl'. $min .'.css' ), array(), self::$version, 'all' );
      }

      // Main scripts
      wp_enqueue_script( 'splwt-lite-plugins', self::include_plugin_url( 'assets/js/plugins'. $min .'.js' ), array(), self::$version, true );
      wp_enqueue_script( 'splwt-lite', self::include_plugin_url( 'assets/js/main'. $min .'.js' ), array( 'splwt-lite-plugins' ), self::$version, true );

      // Main variables
      wp_localize_script( 'splwt-lite', 'csf_vars', array(
        'color_palette'     => apply_filters( 'csf_color_palette', array() ),
        'i18n'              => array(
          'confirm'         => esc_html__( 'Are you sure?', 'splwt-lite' ),
          'typing_text'     => esc_html__( 'Please enter %s or more characters', 'splwt-lite' ),
          'searching_text'  => esc_html__( 'Searching...', 'splwt-lite' ),
          'no_results_text' => esc_html__( 'No results found.', 'splwt-lite' ),
        ),
      ) );
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field['_notice'] ) ) {
        ...
        $field            = array();
        $field['content'] = esc_html__( 'Oops! Not allowed.', 'splwt-lite' ) .' <strong>('. $field_type .')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      ...
      if ( ! empty( $field_type ) ) {
        ...
        if ( class_exists( $classname ) ) {
          ...
          echo '<p>'. esc_html__( 'Field not found!', 'splwt-lite' ) .'</p>';
        }
