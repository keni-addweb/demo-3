...
class Shortcode {
	 ...
	 *
	 * @param array  $data The place to get weather information for.
	 ...
	 */
	public function current_weather_data( $data, $time_format, $temperature_scale, $wind_speed_unit, $weather_units ) {
		$city        = $data->city->name;
		...
		if ( '12' === $time_format ) {
			...
			$time_format  = $last_update->format( ' g:i a,  M d' );
			$sunrise_time = $sunrise->format( 'g:i a' );
			$sunset_time  = $sunset->format( ' g:i a' );
		} else {
			// 12-hour time to 24-hour time
			$time_format  = $last_update->format( ' H:i,  M d' );
	 ...
	 * @param array|int|string $data The place to get weather information for. For possible values see below.
	...
	private function parse_xml( $answer ) {
		...
		try {
			...
			if ( isset( $error['message'] ) ) {
				throw new LWException( $error['message'], isset( $error['cod'] ) ? $error['cod'] : 0 );
			} else {
				throw new LWException( 'Unknown fatal error: OpenWeatherMap returned the following json object: ' . $answer );
			}
		}
	}
